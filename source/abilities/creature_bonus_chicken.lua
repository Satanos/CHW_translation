...
function modifier_creature_bonus_chicken:OnCreated( kv )
	...
	if IsServer() then
		...
		self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_kill", {duration = self.time_limit})
	end
end
...
function modifier_creature_bonus_chicken:OnIntervalThink()
	if IsServer() then
		...
		if GameRules:GetGameTime() > self.flExpireTime then
			local tower = Entities:FindByNameNearest( "dota_badguys_tower2_mid", self:GetParent():GetOrigin(), 99999 )
