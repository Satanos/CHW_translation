...
function earthshaker_echo_slam_lua:OnSpellStart()
	if IsServer() then
		...
		local total = (#enemies*self:GetSpecialValueFor( "echo_slam_echo_damage" )) + self:GetAbilityDamage()
		if total and total >= 6000 then
			local pID = self:GetCaster():GetPlayerOwnerID()
			print(total)
			if Util:IsPremium(self:GetCaster():GetPlayerOwnerID()) and GameRules.Globals.Quests[pID] then
				local damage = math.floor(total)
				GameRules.Globals.Quests[pID].progress = (GameRules.Globals.Quests[pID].progress or 0) + damage
				print(GameRules.Globals.Quests[pID].progress)
				Quests:UpdateQuest(pID)
			end
		end
	end
end
