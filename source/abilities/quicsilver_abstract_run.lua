...
function modifier_quicsilver_abstract_run:OnIntervalThink()
   if IsServer() then
     ...
     local damage = (self:GetCaster():GetIdealSpeed() - target:GetIdealSpeed() ) / 9
     if damage <= 0 then
         damage = (self:GetAbility():GetSpecialValueFor("damage")/100) * self:GetParent():GetMaxHealth()
     else
         damage = ((self:GetAbility():GetSpecialValueFor("damage")/100) * self:GetParent():GetMaxHealth()) + damage
     end
