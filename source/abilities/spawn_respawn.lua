...
function modifier_spawn_respawn:OnTakeDamage(args)
    if self:GetParent() == args.unit then
        if IsServer() then
            ...
            if self:GetParent():GetHealth() <= 0 and self:GetAbility():IsCooldownReady() and self:GetParent():GetMana() > 180 then
...
function modifier_spawn_respawn_buff:GetModifierExtraHealthBonus( params )
